...
const pravoTrainings = SCHEDULE_DATA.filter((item) =>
  isPravoTraining(item.name)
);

const programAreas = [
  {
    title: "Трудовое право",
    desc: "Трудовой кодекс РК, трудовые споры, медиация, вахтовый метод",
    category: "labor",
    color: "#3B82F6",
    keywords: ["трудов", "медиац", "спор", "консенсус", "согласительн", "вахт"],
    icon: (
  ...
  },
  {
    ...
    desc: "Составление и экспертиза договоров, претензионная работа",
    category: "contracts",
    color: "#10B981",
    keywords: ["договор", "контракт", "претензи", "обязательств"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    desc: "Налоговый кодекс, НДС, трансфертное ценообразование, налоговые проверки",
    category: "tax",
    color: "#F59E0B",
    keywords: ["налог", "НДС", "трансфертн"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    desc: "Корпоративное управление, комплаенс, антикоррупция",
    category: "corporate",
    color: "#8B5CF6",
    keywords: ["корпоративн", "комплаенс", "антикоррупц", "лицензи"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {
    title: "Недропользование и экология",
    desc: "Закон о недрах, экологическое законодательство, разрешительные процедуры",
    category: "subsoil",
    color: "#EF4444",
    keywords: ["недропользован", "экологическ", "земельн", "разрешени"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {
    title: "Госзакупки и ВЭД",
    desc: "Государственные закупки, таможенное регулирование, экспорт-импорт",
    category: "procurement",
    color: "#06B6D4",
    keywords: ["госзакуп", "тендер", "таможен", "ВЭД", "экспорт", "импорт", "субподряд"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function getTrainingsByCategory(category: string): typeof pravoTrainings {
  const area = programAreas.find((a) => a.category === category);
  if (!area) return pravoTrainings;
  return pravoTrainings.filter((t) => {
...
const TREND_KEYWORDS = ["ИИ", "искусственн", "AI", "цифров", "LegalTech", "электронн"];
...
const testimonials = [
  {
    quote: "Курсы по трудовому законодательству помогли нам полностью перестроить кадровую документацию. За год — ни одного штрафа от трудовой инспекции",
  ...
  {
    quote: "Семинар по договорной работе сэкономил нам миллионы тенге. Теперь каждый контракт проходит через выстроенную систему правовой экспертизы",
    name: "Ерлан Касымов",
  ...
  {
    quote: "Курс по госзакупкам дал нашей команде полное понимание процедур. Количество успешных тендеров выросло на 60%",
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold mb-4" style={{ color: "#ffffff", textShadow: "0 4px 12px rgba(0,0,0,0.6)" }}>
            От <span className="text-[#F0BB1E]">трудового права</span> до{" "}
            <span className="text-[#00767D]">госзакупок</span>
          </h2>
          <p className="text-lg text-[#B8CDD0] max-w-2xl mx-auto">
            Полный цикл правового обучения для бизнеса
          </p>
        </div>
    ...
    try {
      await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          name: fd.get("name"),
          phone: fd.get("phone"),
          message: `[Право: ${training.name}] Дата: ${training.date}, Компания: ${fd.get("company") || "—"}`,
        }),
      });
    } catch { /* ok */ }
    setStatus("success");
      ...
      <div className="relative z-10">
        <div className="text-center mb-10 px-4">
          ...
          <p className="text-[#546569] max-w-2xl mx-auto text-lg scroll-fade-in scroll-delay-2">
            Крупнейшие компании Казахстана выбрали нас для обучения юридических команд
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-8 max-w-3xl mx-auto text-center px-4">
          <div className="p-3 md:p-6 rounded-2xl bg-white shadow-sm border border-[#00767D]/10 scroll-fade-in">
            <div className="text-2xl sm:text-3xl md:text-5xl font-bold text-[#00767D] mb-1 md:mb-2">25+</div>
    ...
    try {
      await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          name: fd.get("name"),
          phone: fd.get("phone"),
          message: `[Юридические тренинги] Компания: ${fd.get("company") || "—"}, Сотрудников: ${fd.get("employees") || "—"}, Направление: ${fd.get("direction") || "—"}`,
        }),
      });
    } catch { /* ok */ }
        ...
        >
          <div className="max-w-4xl mx-auto">
            ...
            <nav className="flex items-center gap-2 text-sm text-[#7A9EA3] mb-8 scroll-fade-in">
              ...
              <span>/</span>
              <span className="text-[#F0BB1E]">Право</span>
            </nav>
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 text-[#F0BB1E] text-sm font-semibold mb-8 scroll-fade-in backdrop-blur-sm border border-white/10">
              ...
              25+ юридических курсов и семинаров
            </div>
            ...
            <p className="text-lg sm:text-xl text-[#D4E4E7] mb-10 max-w-2xl scroll-fade-in scroll-delay-2">
              От трудового кодекса до госзакупок — практические семинары для юристов, кадровиков и руководителей
            </p>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12 scroll-fade-in">
              ...
              <p className="text-lg text-[#3d5153]">
                {filteredTrainings.length} курсов {activeCategory ? "в выбранной категории" : "по юридической тематике"}
              </p>
            </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12 scroll-fade-in">
              ...
              <p className="text-lg text-[#3d5153]">
                80% практики на реальных кейсах и судебной практике
              </p>
            </div>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid sm:grid-cols-3 gap-6 mb-20">
              {[
                {
                  value: "0",
                  label: "штрафов от инспекций",
                  desc: "после правового аудита",
                  caseStudy: "Провели полный аудит кадровой документации для производственной компании с 800 сотрудниками. Выявили и устранили 47 нарушений. За год — ноль штрафов при трёх проверках.",
                  icon: (
                ...
                {
                  value: "\u2191 60%",
                  label: "успешных тендеров",
                  desc: "после курса по госзакупкам",
                  caseStudy: "Обучили тендерный отдел логистической компании (15 человек). Внедрили систему подготовки конкурсных заявок. Процент побед в тендерах вырос с 25% до 40%.",
                  icon: (
                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {
                  value: "\u2193 80%",
                  label: "правовых рисков",
                  desc: "после обучения по комплаенсу",
                  caseStudy: "Разработали программу комплаенс-обучения для нефтегазовой компании. 200 сотрудников прошли курс. Количество инцидентов, требующих юридического вмешательства, снизилось на 80%.",
                  icon: (
                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="grid lg:grid-cols-2">
          ...
          <div className="bg-gradient-to-br from-[#1a2e30] via-[#0d2628] to-[#0a1f21] p-10 sm:p-16 lg:p-20">
            <div className="max-w-lg ml-auto">
              ...
              <div className="space-y-6">
                {[
                  "Юристы не успевают за изменениями в законодательстве",
                  "Договоры составляются по шаблонам без учёта рисков",
                  "Трудовые споры заканчиваются не в пользу компании",
                  "Госзакупки проигрываются из-за формальных ошибок",
                ].map((problem, i) => (
          ...
          <div className="bg-[#EEF4F4] p-10 sm:p-16 lg:p-20">
            <div className="max-w-lg">
              <p className="text-[#00767D] text-xs uppercase tracking-widest font-semibold mb-4 scroll-fade-in">Правовая защита бизнеса</p>
              ...
              <div className="space-y-6">
                {[
                  "Актуальные знания с учётом последних изменений НПА",
                  "Договорная работа, минимизирующая риски",
                  "Досудебное урегулирование трудовых споров",
                  "Профессиональная подготовка тендерной документации",
                ].map((solution, i) => (
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-2xl mx-auto">
            <div className="text-center mb-10 scroll-fade-in">
              <h2 className="text-3xl sm:text-4xl font-extrabold mb-4" style={{ color: "#ffffff", textShadow: "0 4px 12px rgba(0,0,0,0.6)" }}>
                Закажите юридическое обучение для вашей команды
              </h2>
              <p className="text-lg text-[#B8CDD0]">
                Подберём программу под вашу специфику — от трудового права до комплаенса
              </p>
            </div>
