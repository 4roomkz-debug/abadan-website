...
const financeTrainings = SCHEDULE_DATA.filter((item) =>
  isFinanceTraining(item.name)
);

const programAreas = [
  {
    title: "Бухгалтерский учёт",
    desc: "Бухучёт, отчётность, первичная документация, 1С",
    category: "accounting",
    color: "#3B82F6",
    keywords: ["бухгалтер", "учёт", "учет", "баланс", "отчётност", "отчетност", "первичн"],
    icon: (
  ...
  {
    title: "МСФО и аудит",
    desc: "Международные стандарты финансовой отчётности, аудит, due diligence",
    category: "ifrs",
    color: "#10B981",
    keywords: ["МСФО", "IFRS", "аудит", "международн стандарт"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    desc: "Налоговый кодекс, НДС, КПН, трансфертное ценообразование",
    category: "tax",
    color: "#F59E0B",
    keywords: ["налог", "НДС", "трансфертн", "ценообразован"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    desc: "Бюджетирование, планирование, контроль затрат, себестоимость",
    category: "budgeting",
    color: "#8B5CF6",
    keywords: ["бюджет", "затрат", "себестоимост", "калькуляц", "планирован"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {
    title: "Казначейство и cashflow",
    desc: "Управление денежными потоками, казначейство, ликвидность",
    category: "treasury",
    color: "#EF4444",
    keywords: ["казначейств", "cash flow", "ликвид", "денежн", "дебитор", "кредитор"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    desc: "Инвестиционный анализ, оценка стоимости бизнеса, финмоделирование",
    category: "investment",
    color: "#06B6D4",
    keywords: ["инвестиц", "оценк стоимост", "бизнес-план", "финансов анализ", "амортизац"],
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function getTrainingsByCategory(category: string): typeof financeTrainings {
  const area = programAreas.find((a) => a.category === category);
  if (!area) return financeTrainings;
  return financeTrainings.filter((t) => {
...
const TREND_KEYWORDS = ["ИИ", "искусственн", "AI", "цифров", "автоматизац", "data", "аналитик", "FinTech"];
...
const testimonials = [
  {
    quote: "Курс по МСФО позволил нашей команде перейти на международные стандарты за 4 месяца. Сэкономили миллионы на консалтинге, сделав всё внутренними силами",
    name: "Жанна Каримова",
  ...
  {
    quote: "После обучения по бюджетированию мы снизили отклонения план-факт с 30% до 8%. Теперь бюджет — реальный инструмент управления, а не формальность",
    name: "Тимур Нуржанов",
  ...
  {
    quote: "Семинар по налоговому кодексу помог нам легально оптимизировать налоговую нагрузку на 15%. Окупился за первый квартал",
    name: "Маржан Сулейменова",
    title: "Руководитель финансового отдела",
...
function ProgramAreas({ onCategoryClick }: { onCategoryClick: (category: string) => void }) {
  return (
    <div className="relative py-20 sm:py-28 overflow-hidden bg-[#f0f9f9]">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold mb-4 text-[#2D3A3C]">
            От <span className="text-[#F0BB1E]">учёта</span> до{" "}
            <span className="text-[#00767D]">инвестиций</span>
          </h2>
          <p className="text-lg text-[#546569] max-w-2xl mx-auto">
            Полный цикл финансового обучения для бизнеса
          </p>
        </div>
    ...
    try {
      await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          name: fd.get("name"),
          phone: fd.get("phone"),
          message: `[Финансы: ${training.name}] Дата: ${training.date}, Компания: ${fd.get("company") || "—"}`,
        }),
      });
    } catch { /* ok */ }
    setStatus("success");
        ...
        {status === "success" ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <input name="name" type="text" placeholder="Ваше имя" required className="w-full px-4 py-3 bg-white/15 border border-white/20 rounded-xl text-white placeholder:text-[#6B9196] focus:outline-none focus:border-white/40 focus:bg-white/20 transition-all text-sm" />
              <input name="phone" type="tel" placeholder="Телефон" required className="w-full px-4 py-3 bg-white/15 border border-white/20 rounded-xl text-white placeholder:text-[#6B9196] focus:outline-none focus:border-white/40 focus:bg-white/20 transition-all text-sm" />
              <input name="company" type="text" placeholder="Компания (необязательно)" className="w-full px-4 py-3 bg-white/15 border border-white/20 rounded-xl text-white placeholder:text-[#6B9196] focus:outline-none focus:border-white/40 focus:bg-white/20 transition-all text-sm" />
      ...
      <div className="relative z-10">
        <div className="text-center mb-10 px-4">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#2D3A3C] mb-4 scroll-fade-in scroll-delay-1">
            Лидеры <span className="text-[#F0BB1E]">доверяют</span> нам
          </h2>
          <p className="text-[#546569] max-w-2xl mx-auto text-lg scroll-fade-in scroll-delay-2">
            Крупнейшие компании Казахстана выбрали нас для обучения финансовых команд
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-8 max-w-3xl mx-auto text-center px-4">
          <div className="p-3 md:p-6 rounded-2xl bg-white shadow-sm border border-[#00767D]/10 scroll-fade-in">
            <div className="text-2xl sm:text-3xl md:text-5xl font-bold text-[#00767D] mb-1 md:mb-2">18+</div>
    ...
    try {
      await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          name: fd.get("name"),
          phone: fd.get("phone"),
          message: `[Финансовые тренинги] Компания: ${fd.get("company") || "—"}, Сотрудников: ${fd.get("employees") || "—"}, Направление: ${fd.get("direction") || "—"}`,
        }),
      });
    } catch { /* ok */ }
    ...
    <div className="min-h-screen bg-white">
      ...
      <section ref={heroRef} className="relative pt-24 pb-16 sm:pt-32 sm:pb-24 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          ...
          >
            <source src="/videos/finansy-hero.mp4" type="video/mp4" />
          </video>
        ...
        >
          <div className="max-w-4xl mx-auto">
            ...
            <nav className="flex items-center gap-2 text-sm text-[#7A9EA3] mb-8 scroll-fade-in">
              ...
              <span>/</span>
              <span className="text-[#F0BB1E]">Финансы</span>
            </nav>
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 text-[#F0BB1E] text-sm font-semibold mb-8 scroll-fade-in backdrop-blur-sm border border-white/10">
              ...
              18+ финансовых курсов и семинаров
            </div>
            ...
            <p className="text-lg sm:text-xl text-[#D4E4E7] mb-10 max-w-2xl scroll-fade-in scroll-delay-2">
              От бухгалтерского учёта до МСФО — практические курсы для финансистов, бухгалтеров и руководителей
            </p>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12 scroll-fade-in">
              ...
              <p className="text-lg text-[#3d5153]">
                {filteredTrainings.length} курсов {activeCategory ? "в выбранной категории" : "по финансовой тематике"}
              </p>
            </div>
              ...
              {[
                {
                  ...
                  desc: "после курса по оптимизации",
                  caseStudy: "Провели налоговый аудит для торговой сети с оборотом 5 млрд тенге. Обучили финансовый отдел (12 человек) легальным методам оптимизации. Экономия за год — более 750 млн тенге.",
                  icon: (
                ...
                },
                {
                  ...
                  desc: "после курса по бюджетированию",
                  caseStudy: "Внедрили систему rolling forecast для горнодобывающей компании. Обучили 30 специалистов по бюджетированию. Отклонение план-факт снизилось с 30% до 8% за два квартала.",
                  icon: (
                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                    </svg>
                ...
                {
                  value: "4 мес.",
                  label: "переход на МСФО",
                  desc: "вместо обычных 12 месяцев",
                  caseStudy: "Подготовили финансовую команду промышленного холдинга (20 человек) к переходу на МСФО. Обычно переход занимает год, но благодаря интенсивному обучению компания завершила его за 4 месяца.",
                  icon: (
                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid lg:grid-cols-2 gap-0 rounded-2xl overflow-hidden shadow-lg scroll-fade-in scroll-delay-1">
              ...
              <div className="bg-white p-8 sm:p-10 lg:p-12">
                ...
                <div className="space-y-5">
                  {[
                    "Финансовая отчётность не отражает реальной картины",
                    "Налоговые проверки выявляют нарушения",
                    "Бюджет — формальность, а не инструмент управления",
                    "Нет специалистов для перехода на МСФО",
                  ].map((problem, i) => (
              ...
              <div className="bg-[#00767D] p-8 sm:p-10 lg:p-12">
                ...
                <div className="space-y-5">
                  {[
                    ...
                    "Бюджетирование с контролем отклонений",
                    "Внутренние эксперты по МСФО и аудиту",
                  ].map((solution, i) => (
          ...
          <div className="max-w-2xl mx-auto">
            <div className="text-center mb-10 scroll-fade-in">
              <h2 className="text-3xl sm:text-4xl font-extrabold mb-4 text-white">
                Закажите финансовое обучение для вашей команды
              </h2>
              <p className="text-lg text-white/80">
                Подберём программу под ваши задачи — от бухучёта до инвестиционного анализа
              </p>
            </div>
